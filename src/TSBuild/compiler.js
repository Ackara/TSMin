...
function readConfigurationFile() {
	...
	config.getFileList = function (batch) {
		...
		for (var i = 0; i < batch.include.length; i++) {
			var result = glob.sync(batch.include[i], { cwd: path.dirname(fullPath) });
			
			for (var y = 0; y < result.length; y++) {
				list.push(result[y]);

				// The system tend to stall here when too much data is written to the console, so I am limiting it.
				if (y < 10) { console.log("<<" + path.resolve(result[y])); }
				else if (y === 10) { console.log("<<..."); }
			}
		}
		return list;
	}
